...
class QuestionModel {
     ...
     * @param {number|string} examId - The exam ID or 'bank' for question bank
        ...
        try {
            ...
            if (files && files.length > 0) {
                for (const file of files) {
                    const mediaQuery = `
                        ...
                        VALUES (${questionId}, ${this.sqlQuote(file.originalname)}, ${this.sqlQuote(file.path)}, NULL, NULL, GETDATE())
                    `;
            ...
            if (questionData.options) {
                ...
                for (const option of options) {
                    const optText = this.sqlQuote(option.text || option.option || '');
                    const optExpl = option.explanation ? this.sqlQuote(option.explanation) : 'NULL';
                    const optionQuery = `
     ...
     * @returns {Object} - Result object with success status
     */
    static async deleteMCQQuestion(questionId) {
        try {
